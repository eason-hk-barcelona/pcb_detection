...
class OpenGaussModel:
    ...
    def connect(self):
        conn = psycopg2.connect(
            dbname=os.environ["OG_DB_NAME"],
            user=os.environ["OG_DB_USER"],
            password=os.environ["OG_DB_PASSWORD"],
            host=os.environ["OG_DB_HOST"],
            port=os.environ["OG_DB_PORT"]
        )
        return conn
